@props(['produk'])

<!-- Modal toggle -->
<div class="p-3">
    <!-- Tombol Edit -->
    <button data-modal-target="modal-edit-{{ $produk->id }}" 
        data-modal-toggle="modal-edit-{{ $produk->id }}" 
        type="button">
        <img src="{{ asset('images/edit.svg') }}" alt="edit" class="w-5 h-5 cursor-pointer">
    </button>

    <!-- Tombol Hapus -->
    <button data-modal-target="delete-modal-{{ $produk->id }}" data-modal-toggle="delete-modal-{{ $produk->id }}" type="button">
        <img src="{{ asset('images/delete.svg') }}" alt="delete" class="w-5 h-5 cursor-pointer">
    </button>
</div>

<!-- Modal Delete -->
<div id="delete-modal-{{ $produk->id }}" tabindex="-1" aria-hidden="true"
    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative rounded-lg shadow-sm dark:bg-gray-700">
        <div class="relative bg-gray-700 rounded-lg shadow-sm">
            <button type="button"
                class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                data-modal-hide="delete-modal-{{ $produk->id }}">
                <svg class="w-3 h-3" aria-hidden="true" fill="none" viewBox="0 0 14 14">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                </svg>
                <span class="sr-only">Close modal</span>
            </button>
            <div class="p-4 md:p-5 text-center">
                <svg class="mx-auto mb-4 text-gray-400 w-12 h-12" fill="none" viewBox="0 0 20 20">
                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M10 11V6m0 8h.01M19 10a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
                </svg>
                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah anda yakin ingin menghapus produk ini?</h3>
                <form action="{{ route('deleteProduk', ['id' => $produk->id]) }}" method="POST">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
                        class="text-white mb-3 bg-red-600 hover:bg-red-800 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5">
                        Ya, saya yakin
                    </button>
                </form>
                <button data-modal-hide="delete-modal-{{ $produk->id }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100
                    dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Tidak, batal</button>
            </div>
        </div>
    </div>
</div>

<!-- Modal Edit -->
<div id="modal-edit-{{ $produk->id }}" tabindex="-1" aria-hidden="true"
    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
    <div class="relative p-4 w-full max-w-md max-h-full">
        <div class="relative bg-white dark:bg-gray-300 rounded-lg shadow-sm">
            <div class="flex items-center justify-between p-4 md:p-5 border-b rounded-t border-gray-200">
                <h3 class="text-lg font-semibold text-gray-800">Edit Produk</h3>
                <button type="button"
                    class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center"
                    data-modal-hide="modal-edit-{{ $produk->id }}">
                    <svg class="w-3 h-3" aria-hidden="true" fill="none" viewBox="0 0 14 14">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    </svg>
                </button>
            </div>
            <form class="p-4 md:p-5">
                <div class="grid gap-4 mb-4 grid-cols-1">
                    <div>
                        <label for="nama-{{ $produk->id }}" class="block mb-2 text-sm font-medium text-gray-800">
                            Nama Produk
                        </label>
                        <input type="text" name="nama" id="nama-{{ $produk->id }}"
                            value="{{ $produk->nama }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5"
                            required>
                    </div>
                    <div>
                        <label for="gambar-{{ $produk->id }}" class="block mb-2 text-sm font-medium text-gray-800">
                            Gambar
                        </label>
                        <input type="file" id="gambar-{{ $produk->id }}" name="gambar"
                        class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50">
                    </div>
                    <div>
                        <label for="detail-{{ $produk->id }}" class="block mb-2 text-sm font-medium text-gray-800">
                            Deskripsi
                        </label>
                        <textarea name="detail" id="detail-{{ $produk->id }}"
                            class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5 h-28"
                            required>{{ $produk->detail }}</textarea>
                    </div>
                </div>
                <div class="flex justify-end border-t border-gray-200 p-4">
                    <button type="submit"
                        class="text-white bg-blue-700 hover:bg-blue-800 font-medium rounded-lg text-sm px-5 py-2.5 mr-2">
                        Simpan
                    </button>
                    <button data-modal-hide="modal-edit-{{ $produk->id }}" type="button"
                        class="text-gray-900 bg-white border border-gray-300 hover:bg-gray-100 font-medium rounded-lg text-sm px-5 py-2.5">
                        Batal
                    </button>
                </div>
            </form>
            
        </div>
    </div>
</div>
