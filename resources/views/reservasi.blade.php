    ...
    <form action="{{ route('reservasi.store') }}" method="POST">
        ...
        <div class="max-w-4xl mx-auto bg-white p-6 rounded-lg border border-blue-200 shadow-[0px_0px_10px_rgba(59,130,246,0.3)] font-['Inter'] mb-20 relative">
            ...
            <div class="md:col-span-2 mb-3">
                ...
                <input type="text" disabled name="promo" placeholder="Masukkan promo" value="{{ $promo }}"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select id="treatmentSelect" name="perawatan" class="w-full mt-1 px-4 py-2 border border-gray-400 rounded-lg focus:outline-blue-200 appearance-none">
                        <option value="">Pilih perawatan</option>
                        <option value="vaksinasi" {{ $perawatan == 'vaksinasi' ? 'selected' : '' }}>Vaksinasi</option>
                        <option value="operasi-minor-mayor" {{ $perawatan == 'operasi-minor-mayor' ? 'selected' : '' }}>Operasi minor & mayor</option>
                        <option value="cek-darah" {{ $perawatan == 'cek-darah' ? 'selected' : '' }}>Cek Darah</option>
                        <option value="cek-mikroskopis" {{ $perawatan == 'cek-mikroskopis' ? 'selected' : '' }}>Cek Mikroskopis</option>
                        <option value="rawat-inap" {{ $perawatan == 'rawat-inap' ? 'selected' : '' }}>Rawat Inap</option>
                        <option value="usg" {{ $perawatan == 'usg' ? 'selected' : '' }}>USG</option>
                        <option value="pengobatan" {{ $perawatan == 'pengobatan' ? 'selected' : '' }}>Pengobatan</option>
                    </select>
                </div>
                  ...
                  <div>
                    ...
                    <div class="relative max-w-sm">
                        ...
                        <input id="datepicker" name="tanggal" type="text" class="border border-gray-400 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block ps-10 w-104 py-2 mt-1 dark:placeholder-gray-400 dark:text-gray-800 dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Pilih tanggal">
                    </div>
                </div>

                <div>
                    <label class="block text-gray-700">Waktu</label>
                    <select id="timeSelect" name="waktu" class="w-full mt-1 px-4 py-2 border border-gray-400 rounded-lg focus:outline-blue-200 appearance-none">
                ...
                <div>
                    ...
                    <select id="doctorSelect" name="dokter" class="w-full mt-1 px-4 py-2 border border-gray-400 rounded-lg focus:outline-blue-200 appearance-none">
                        <option value="">Pilih dokter</option>
                        <option value="Drh. Siti">Drh. Siti</option>
                        <option value="Drh. Andi">Drh. Andi</option>
                        <option value="Drh. Rina">Drh. Rina</option>
                    </select>
                </div>

                <div class="md:col-span-2">
                    <label class="block text-gray-700">Pesan</label>
                    <textarea name="pesan" placeholder="Masukkan Pesan" class="w-full row-2 mt-1 px-4 py-2 border border-gray-400 rounded-lg focus:outline-blue-200"></textarea>
                </div>
            </div>

            <div class="flex justify-end space-x-4 mt-6">
                <button type="submit" class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg">Kirim</button>
                <button type="reset" class="px-6 py-2 bg-blue-100 hover:bg-blue-200 text-blue-800 font-medium rounded-lg">Batal</button>
            </div>
        </div>
    </form>
</x-layout>
